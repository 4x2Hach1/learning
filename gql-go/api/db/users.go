...
type userR struct {
	Memories MemorySlice `boil:"Memories" json:"Memories" toml:"Memories" yaml:"Memories"`
}
...
func (r *userR) GetMemories() MemorySlice {
	if r == nil {
		return nil
	}
	return r.Memories
...
// Memories retrieves all the memory's Memories with an executor.
func (o *User) Memories(mods ...qm.QueryMod) memoryQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`memories`.`users_id`=?", o.ID),
	)

	return Memories(queryMods...)
}

// LoadMemories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadMemories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`memories`),
		qm.WhereIn(`memories.users_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load memories")
	}

	var resultSlice []*Memory
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice memories")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on memories")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UsersID {
				local.R.Memories = append(local.R.Memories, foreign)
				if foreign.R == nil {
					foreign.R = &memoryR{}
				}
...
// AddMemories adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Memories.
// Sets related.R.User appropriately.
func (o *User) AddMemories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Memory) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Memories = append(o.R.Memories, related...)
	}
